...
naughty.config.defaults.font = "文泉驿微米黑 12"
...
config_dir = ("/home/worry/.config/awesome/")
...
for s = 1, screen.count() do
    ...
    tags[s] = awful.tag({ "Net", "Code", "SDK" , "Doc", "Video", "Game", 7, 8, 9 }, s, layouts[3])
end
-- }}}
...
mymainmenu = awful.menu({ items = { { "awesome", myawesomemenu, beautiful.awesome_icon },
                                    ...
                                    { "log out", "/home/worry/.config/awesome/haltmenu.sh" },
                                  }
                        })
...
function update_batwidget()
    ...
    for line in f:lines() do
	    percent = line:match('^Battery [^:]+:%s+%a+,%s+(%d+)%%')
	    state = line:match('^Battery [^:]+:%s+(%a+),%s+%d+%%')
    end 
...
function display_batstates()
    ...
    for line in f:lines() do
	    percent = line:match('^Battery [^:]+:%s+%a+,%s+(%d+)%%')
	    state = line:match('^Battery [^:]+:%s+(%a+),%s+%d+%%')
	    last = line:match('^Battery [^:]+:%s+%a+,%s+%d+%%,%s([%d:]+)')
    end 
...
awful.button({ }, 1, display_batstates )))
...
function display_cputemp()
    ...
    for line in pipe:lines() do
        --temp0 = line:match('^Core [^:]+:%s+%+([0-9.]+)°C')
        --temp1 = line:match('^Core%s+1:%s+%+([0-9.]+)°C')
        local newtemp0 = line:match('^Core%s0:%s+%+([0-9.]+)°C')
        local newtemp1 = line:match('^Core%s1:%s+%+([0-9.]+)°C')
        if newtemp0 then
            newtemp0 = tonumber(newtemp0)
            if temp0 < newtemp0 then
                temp0 = newtemp0
            end
        end
        if newtemp1 then
            newtemp1 = tonumber(newtemp1)
            if temp1 < newtemp1 then
                temp1 = newtemp1
            end
        end
    end
...
awful.button({ }, 1, display_cputemp )))
...
awful.key({ "Control", modkey }, "c", function() awful.util.spawn("chromium") end),
awful.key({ "Control", modkey }, "i", function() awful.util.spawn("chromium -incognito") end),

-- {{ Shuts down Computer }} --

awful.key({ "Control", "Shift"}, "Escape", function() awful.util.spawn("systemctl poweroff") end),
...
awful.key({ "Control", modkey }, "a", function() awful.util.spawn("amarok") end),
...
awful.key({ modkey,     }, "Up", function() awful.util.spawn("amixer set Master 5%+", false) end),
awful.key({ modkey,     }, "Down", function() awful.util.spawn("amixer set Master 4%-", false) end),
...
clientkeys = awful.util.table.join(
    ...
    awful.key({ modkey,           }, "m",
	    ...
	    if (c.maximized_horizontal ~= c.maximized_vertical) then
